...
Challenge: using prior knowledge of decision statements and looking up boolean operators in the course materials (try NOTES-Cbasics), update your program so that it doesn't crash in the above situations. It is up to you how to handle the various errors types. (In homework, we will specify how to handle various input errors with different return values.)
