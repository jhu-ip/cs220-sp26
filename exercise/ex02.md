...
If there is time, try your solution to the above with different input types. What happens if you enter floating point values? characters? strings? What about using 0 as the second input value?

Challenge: using prior knowledge of decision statements and looking up boolean operators in the course materials (try NOTES-Cbasics), update your program so that it doesn't crash in the above situations. It is up to you how to handle the various types of errors. (In homework, we will specify how to handle various input errors, including with different return values.)
